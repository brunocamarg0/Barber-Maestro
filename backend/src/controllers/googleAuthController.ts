...
/**
 * Callback do OAuth Google para Cliente
 */
export async function googleCallbackCliente(req: Request, res: Response) {
  try {
    ...
    const { googleId, nome, email, foto } = profile;
    ...
    if (!cliente) {
      ...
      // Atualizar cliente existente com Google ID
    ...
    } else if (cliente.googleId !== googleId) {
      // Email já existe mas com outro Google ID
    ...
    console.error('Erro no callback Google Cliente:', error);
    res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}/login?error=google_auth_error`);
  }
}

/**
 * Callback do OAuth Google para Dono
 */
export async function googleCallbackDono(req: Request, res: Response) {
  try {
    const profile: any = req.user;
    ...
    } else if (!dono.googleId) {
      // Atualizar dono existente com Google ID
    ...
    console.error('Erro no callback Google Dono:', error);
    res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}/login?error=google_auth_error`);
  }
}
